...
## 2026-08-10 (раздел событий в админке)

### Added

- **Событие создаётся сразу с датой проведения** — раньше дату можно было добавить только после сохранения, отдельным шагом внизу страницы. Поле обязательное и предзаполнено ближайшим слотом в 19:00 (сегодня, а после 19:00 — завтра). Дата в прошлом разрешена, но помечается предупреждением
- **Список событий показывает, когда они проводятся**: колонки «Ближайшая дата» и «Даты» (сколько предстоящих из скольких, сколько отменено), сортировка по ближайшей дате вместо даты создания, фильтры «Предстоящие / Прошедшие / Без даты / Все» со счётчиками и поиск по названию
- **«+1 неделя»** в списке дат — добавляет дату на неделю позже последней, копируя заметку. Для еженедельных покатушек расписание набирается кликами вместо ручного ввода каждой даты
- Подтверждение удаления даты: раньше один клик сносил дату вместе с участниками и историей анонсов
- **Единственную дату удалить нельзя** — кнопка заблокирована с пояснением. Событие без дат пропадает из ленты и не анонсируется; чтобы свернуть событие, дату переносят, отменяют или удаляют событие целиком

### Changed

- Прошедшие даты события свёрнуты за «Показать прошедшие (N)» — у еженедельного события их со временем десятки
- Событие, у которого все даты отменены, попадает в фильтр «Без даты», а не выглядит запланированным
- `listEvents` забирает даты тем же запросом (без N+1), `createEvent` принимает первую дату и откатывает событие, если её не удалось вставить
- Полям формы события проставлены `id`/`htmlFor` — лейблы наконец связаны с полями

### Fixed

- Ошибка переключения видимости события показывается в интерфейсе, а не в `window.alert`
- Дефолтная дата в списке дат больше не оказывается в прошлом: после 19:00 предлагался сегодняшний вечер, который уже наступил
- ESLint игнорирует `supabase/.temp`, Prettier — `.playwright-mcp`: при поднятом локальном стеке и работе через Playwright MCP оба гейта падали на чужих временных файлах и роняли pre-commit
