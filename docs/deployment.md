...
- **Фото точки идёт в превью только если легче 600 КБ** (проверяется HEAD-запросом). WhatsApp на картинках тяжелее часто не рисует превью вообще, а фото загружаются в Storage как есть: трансформация (`/storage/v1/render/image/...`) на текущем плане Supabase отдаёт 403. Сейчас под лимит попадает меньшинство фотографий — радикальное решение это либо сжатие при загрузке в админке, либо генерация превью-скриншотов карты.
