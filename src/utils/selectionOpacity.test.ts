...
import { DIM_OPACITY, buildSelectionOpacityExpression, type SelectedFeatureState } from '@/utils/selectionOpacity'
...
describe('selectionOpacity', () => {
    ...
    it('оставляет выбранный элемент непрозрачным и снижает остальные в том же слое', () => {
        ...
        expect(sameSourceExpr).toEqual(['case', ['==', ['id'], 'p-1'], 1, DIM_OPACITY])
    })
    ...
    it('для Telegram-слоя с id telegram-user-N использует свойство telegramUserId', () => {
        ...
        expect(expr).toEqual(['case', ['==', ['get', 'telegramUserId'], 7], 1, DIM_OPACITY])
    })
})
