...
function buildTelegramSelectionExpression(selected: SelectedFeatureState): ExpressionSpecification {
    ...
    if (selectedTelegramUserId !== null) {
        return ['case', ['==', ['get', 'telegramUserId'], selectedTelegramUserId], 1, DIM_OPACITY]
    }
    return ['case', ['==', ['id'], selected.id], 1, DIM_OPACITY]
}
...
export function applySelectionOpacityById(map: MapboxMap | null, selected: SelectedFeatureState | null) {
    if (!map) return
    ...
    if (map.getLayer(LAYER_IDS.telegramUsers)) {
        ...
        const telegramSelectedExpression =
            selected && selected.sourceId === telegramSourceId ? buildTelegramSelectionExpression(selected) : null
        map.setPaintProperty(
            LAYER_IDS.telegramUsers,
            'circle-opacity',
            telegramSelectedExpression
                ? telegramSelectedExpression
                : ['interpolate', ['linear'], ['get', 'ageMinutes'], 0, 1, 10, 0.25],
        )
    }
}
