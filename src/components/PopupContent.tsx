...
export function PopupContent({ feature, onCopied, relatedEvents, onOpenEvents }: PopupContentProps) {
    ...
    const photos =
        feature.properties.type === 'point' || feature.properties.type === 'socket'
            ? (feature.properties.photos ?? [])
            : []
    const telegramAvatarUrl = feature.properties.type === 'telegramUser' ? feature.properties.avatarUrl : null
    ...
    return (
        <div className="text-left">
            <div className="p-2 pb-0">
                ...
                <h3 className="font-semibold text-neutral-900 mt-2 text-[15px]">{name || 'Без названия'}</h3>
                {telegramAvatarUrl && (
                    <img
                ...
                {feature.properties.type === 'telegramUser' && (
                    <>
                        ...
                        {typeof feature.properties.avgSpeedKmh === 'number' &&
                            Number.isFinite(feature.properties.avgSpeedKmh) && (
                                <p className="text-xs text-neutral-500 mt-0.5">
                                    Средняя скорость (по последним {feature.properties.avgSpeedWindowPoints ?? 5}{' '}
                                    точкам): {feature.properties.avgSpeedKmh.toFixed(1)} км/ч
                                </p>
                            )}
                    </>
                )}
