...
export function PointEventsBlock({ events, onOpenEvents }: PointEventsBlockProps) {
    const items = useMemo(() => events.map((event) => ({ event, summary: summarizeEvent(event) })), [events])
    return (
                        ...
                        >
                            ...
                            {summary.next && (
                                <div className="mt-0.5 text-[11px] text-neutral-500">
                                    {summary.ongoing ? 'Следующее' : 'Ближайшее'}:{' '}
                                    {formatOccurrenceLabel(summary.next.start, undefined, event.duration_minutes)}
                                </div>
                            )}
                        </button>
                    </li>
