...
export function FeatureSidebar({ feature, onClose, relatedEvents, onOpenEvents }: FeatureSidebarProps) {
    return (
        ...
        bottom-0 left-0 right-0 max-h-[40vh] rounded-t-2xl border-t animate-slide-up
        pl-[max(0px,env(safe-area-inset-left))] pr-[max(0px,env(safe-area-inset-right))]
        pb-[max(0px,env(safe-area-inset-bottom))]
        md:bottom-0 md:top-0 md:left-auto md:right-0 md:max-h-none md:h-full md:w-[320px] md:max-w-[90vw] md:rounded-none md:rounded-l-2xl md:border-t-0 md:border-l md:animate-slide-in-right"
            role="dialog"
        ...
        >
            <div className="flex items-center justify-between shrink-0 px-4 py-2.5 border-b border-neutral-200/80 bg-neutral-50/80">
                <span className="text-xs font-medium text-neutral-500 uppercase tracking-wider">Подробности</span>
                <button
                    type="button"
                    onClick={onClose}
