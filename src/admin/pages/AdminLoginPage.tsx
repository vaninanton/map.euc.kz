...
export function AdminLoginPage() {
    ...
    const handlePasskeyLogin = async () => {
        ...
        try {
            // При успехе SDK сам сохранит сессию и эмитит SIGNED_IN — AdminAuthGate перерисуется.
        ...
        <div className="flex min-h-dvh flex-col items-center justify-center gap-6 bg-neutral-50 p-6">
            <div className="w-full max-w-sm rounded-xl border border-neutral-200 bg-white p-6 shadow-sm">
                ...
                <p className="mt-1 text-sm text-neutral-600">
                    Аккаунт Supabase Auth с правами в таблице{' '}
                ...
                {passkeySupported ? (
                    <div className="mt-4">
                        <button
                            ...
                            className="w-full cursor-pointer rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-blue-700 disabled:cursor-not-allowed disabled:bg-blue-300"
                        >
                            {passkeyLoading ? 'Подтвердите на устройстве…' : 'Войти по пасскею'}
                        </button>
                        <p className="mt-2 text-xs text-neutral-500">
                            Face ID / Touch ID / PIN устройства. Пасскей добавляется в админке: Настройки → Пасскеи.
                        </p>
                    </div>
                ) : (
                ...
                {!passwordFormOpen && (
                    <div className="mt-4 border-t border-neutral-200 pt-3">
                        <button
                            ...
                            className="cursor-pointer text-xs font-medium text-neutral-500 hover:text-neutral-800"
                        >
                ...
                {passwordFormOpen && (
                    <form className="mt-4 flex flex-col gap-3" onSubmit={(e) => void handleSubmit(e)} noValidate>
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <button
                                    ...
                                    className="absolute right-2.5 top-1/2 -translate-y-1/2 cursor-pointer text-neutral-400 hover:text-neutral-600"
                                >
                                    {showPassword ? (
                                        <svg
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="cursor-pointer rounded-lg bg-neutral-800 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-neutral-900 disabled:cursor-not-allowed disabled:bg-neutral-400"
                        >
                            {loading ? 'Вход…' : 'Войти'}
                        </button>
                    </form>
