...
export function useMapPopup(map: mapboxgl.Map | null, options: UseMapPopupOptions) {
    ...
    const openPopup = useCallback(
        (feature: Feature, lngLat: [number, number]) => {
            ...
            const popup = new mapboxgl.Popup({ offset: 15, closeButton: true }).setLngLat(lngLat).setDOMContent(div)
